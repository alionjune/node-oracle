// ==============================================================
// ORACLE, ODBC and DB2/CLI Template Library, Version 4.0.172,
...
#define OTL_VERSION_NUMBER (0x0400ACL)
...
/* 支持地理空间数据 */
/*************fix for geometry***********/
const int  inGeometry =108;
const int  extGeometry=108;
...
struct SDO_GEOMETRY_BIN
{
   SDO_GEOMETRY_TYPE* p_obj;
   SDO_GEOMETRY_ind*  p_ind;
};
...
struct sdo_geometry_obj
{ 
	 int         gtype_;
	 int         srid_;
	 sdo_point   point_;
	 ::std::vector<int>   elemInfoQueue_;
	 ::std::vector<double>   ordinatesQueue_;
};
...
/******fix for geometry******/
const int otl_var_geometry = 101;
...
inline const char* otl_var_type_name(const int ftype)
{
  ...
  /********fix for geometry***********/
  const char* const_GEOMETRY="GEOMETRY";
  /********fix for geometry***********/
#if defined(OTL_ORA_UNICODE)||defined(OTL_ORA_UTF8)
  ...
  case otl_var_geometry:
	return const_GEOMETRY;
  case 108:
...
public:
 ...
 //   return TConnectStruct::initialize(threaded_mode);
	...
	/* 该函数无须调用下面的函数使用OCIEnvCreate来进行环境初始化  */ 
	return 1;
 }
...
public:
 ...
 int vl_len;
 otl_tmpl_variable<TVariableStruct>** vl;//绑定类型
...
public:
  ...
  *tac=0;
  ...
  case 'D':
   ...
   break;
/************fix for geometry********************/
#if defined(OTL_GEOMETRY)
  case 'G':
	data_type=otl_var_geometry;
	data_len=pdb->get_max_long_size();
	break;
#endif
/************fix for geometry********************/
...
/*****************************************确定占位符变量列表******************************************/
 ...
 (otl_tmpl_variable<TVariableStruct>** &vl,int& vl_len,OTL_TMPL_CONNECT& adb,const int status=def)
 {
  int j;
  vl_len=0;
  if(!hv[0]){
  ...
  int i=0;
  while(hv[i]){
    otl_tmpl_variable<TVariableStruct>* vp = alloc_var(hv[i],inout[i],status,adb,pl_tab_size[i]);
    if(vp==0){
      int j2;
...
public:
 ...
 void get_select_list(void)
 {
   ...
   for(i=1;this->describe_column(sl_desc_tmp[i-1],i);++i){
	 ...
	 /**** fix for select*****/
	 if(sl_desc_tmp[i-1].dbtype == extGeometry)
           ++sl_geom_len;
	 /************************/
     ++sld_tmp_len;
...
#define OTL_D1(T,T_type)                                        \
  ...
  void SDO_geometry_clear(SDO_geometry& g)
  {
       memset(&(g.ind),0,sizeof(sdo_geometry_ind));
	   g.obj.elemInfoQueue_.clear();
	   g.obj.ordinatesQueue_.clear();
  }
  void Geometry_format(SDO_geometry& g,SDO_GEOMETRY_BIN* temp)
  {

	  SDO_geometry_clear(g);

	  if( g.ind.geometryNull_ = (temp->p_ind->_atomic == OCI_IND_NULL))
	      return;
      
	  if(!(g.ind.typeNull_ = (temp->p_ind->sdo_gtype == OCI_IND_NULL)))
	  {
	  OCINumberToInt((this->cursor_struct.db)->errhp, &(temp->p_obj->sdo_gtype),
		  (uword)sizeof(int), OCI_NUMBER_SIGNED,
		  (dvoid *)&g.obj.gtype_);
	  }
	  if(!(g.ind.sridNull_ = (temp->p_ind->sdo_srid == OCI_IND_NULL)))
	  {
		  OCINumberToInt((this->cursor_struct.db)->errhp, &(temp->p_obj->sdo_srid),
			  (uword)sizeof(int), OCI_NUMBER_SIGNED,
			  (dvoid *)&(g.obj.srid_));
	  }

	  if(!(g.ind.pointNull_ = (temp->p_ind->sdo_point._atomic == OCI_IND_NULL)))
	  {
		  if (!(g.ind.point_.x = (temp->p_ind->sdo_point.x == OCI_IND_NULL)))
		  {
			      OCINumberToReal((this->cursor_struct.db)->errhp, &(temp->p_obj->sdo_point.x), 
				  (uword)sizeof(double),
				  (dvoid *)&(g.obj.point_.x));
		  }

		  if (!(g.ind.point_.y = (temp->p_ind->sdo_point.y == OCI_IND_NULL)))
		  {
			OCINumberToReal((this->cursor_struct.db)->errhp, &(temp->p_obj->sdo_point.y), 
				  (uword)sizeof(double),
				  (dvoid *)&(g.obj.point_.y));
		  }

		  if (!(g.ind.point_.z = (temp->p_ind->sdo_point.z == OCI_IND_NULL)))
		  {
			  OCINumberToReal((this->cursor_struct.db)->errhp, &(temp->p_obj->sdo_point.z), 
				  (uword)sizeof(double),
				  (dvoid *)&(g.obj.point_.z));
		  }
	  }

	  if(!(g.ind.elemInfoNull_ = (temp->p_ind->sdo_elem_info == OCI_IND_NULL)))
	  {
		  boolean        exists;
		  ...
		  int count = 0;
		  OCICollSize((this->cursor_struct.db)->envhp, (this->cursor_struct.db)->errhp, 
			  (OCIColl *)(temp->p_obj->sdo_elem_info), 
			  &count); 
		  for(int j=0; j<count; j++)
		  {
			  OCICollGetElem((this->cursor_struct.db)->envhp, (this->cursor_struct.db)->errhp, 
			  (OCIColl *)(temp->p_obj->sdo_elem_info), 
			  (sb4)j, 
			  (boolean *)&exists, 
			  (dvoid **)&oci_number, (dvoid **)0);

		      OCINumberToInt((this->cursor_struct.db)->errhp, oci_number, 
			    (uword)sizeof(ub4), OCI_NUMBER_UNSIGNED,
			    (dvoid *)&element_type);
		      g.obj.elemInfoQueue_.push_back(element_type);
		  }
	  }

	  if(!(g.ind.ordinatesNull_ = (temp->p_ind->sdo_ordinates == OCI_IND_NULL)))
	  {
		  boolean        exists;
		  OCINumber      *oci_number;
		  double         double_val;
		  ...
		  OCICollSize((this->cursor_struct.db)->envhp, (this->cursor_struct.db)->errhp, 
			  (OCIColl *)(temp->p_obj->sdo_ordinates), 
			  &count);
		  ...
		  for(int j=0; j<count; j++)
		  {
			  ...
			  OCINumberToReal((this->cursor_struct.db)->errhp, oci_number, 
				  (uword)sizeof(double),
				  (dvoid *)&double_val);
			  g.obj.ordinatesQueue_.push_back(double_val);
		  }
...
// fix fo select
  OTL_TMPL_SELECT_STREAM& operator>>(SDO_geometry& g)                      
  {                                                            
	  check_if_executed();                                        
	  if(eof_intern())return *this;   
	  // get_next()
	  SDO_GEOMETRY_BIN* temp;
	  if(cur_col<sl_len-1)
	  {
		  ++cur_col;
//		  null_fetched=sl[cur_col].is_null(this->cur_row);
	 ...
	 if(!eof_intern())
	 {   
		 temp = (SDO_GEOMETRY_BIN*)sl[cur_col].val(this->cur_row); 
		 Geometry_format(g,temp);
		 look_ahead();                                        
	 }                                                            
	 return *this;                                        
  }
...
otl_uncaught_exception()){
  ...
  const int rows=0)
 {
  ...
  // fix for geometry by yjq 200-09-04-18
   for(int j=0; j<this->vl_len; j++)
  {
	if(this->vl[j]->ftype == otl_var_geometry)
		for(int i=0; i<array_size; i++)
		{
			OCIObjectFree((this->cursor_struct.db)->envhp,(this->cursor_struct.db)->errhp,
				(dvoid*)(this->vl[j]->var_struct.hyarray[i]),(ub2)OCI_OBJECTFREE_FORCE);
		}
	}
   //fix
 }
 ...
 int check_type(int type_code, int tsize)
 {
   ...
   /**********fix for geometry************************/
   case otl_var_geometry:
	if(type_code==otl_var_geometry)return 1;
 ...
 return *this;                                         \
  ...
  OTL_D3(double,otl_var_double)
    ...
    template<OTL_TYPE_NAME T,const int T_type> OTL_D3(T,T_type)
		...
		/***********fix for geometry******************/
#if defined(OTL_GEOMETRY)
 OTL_TMPL_OUT_STREAM& operator<<(::std::vector<double>& s)
{

	OCINumber oci_number;
	
	OCIObjectNew((this->cursor_struct.db)->envhp,(this->cursor_struct.db)->errhp,(this->cursor_struct.db)->svchp,
		        OCI_TYPECODE_VARRAY,this->cursor_struct.ordinates_tdo,(dvoid*)NULL,OCI_DURATION_SESSION,FALSE,
				(dvoid**)&(this->vl[cur_x+1]->var_struct.hyarray[cur_y]));
	int len =(int)s.size();

	for (int i=0;i<len;i++)
	{
		 OCINumberFromReal((this->cursor_struct.db)->errhp,(dvoid*)&s[i],(uword)sizeof(double),&oci_number);
		 OCICollAppend((this->cursor_struct.db)->envhp,(this->cursor_struct.db)->errhp,
			          (dvoid*)&oci_number,(dvoid*)0,(OCIColl*)(this->vl[cur_x+1]->var_struct.hyarray[cur_y]));
	}
	if(cur_y ==0)
	OCIBindObject(this->vl[cur_x+1]->var_struct.bind,(this->cursor_struct.db)->errhp,
		         this->cursor_struct.ordinates_tdo,(dvoid**)&(this->vl[cur_x+1]->var_struct.hyarray[0]),(ub4*)0,(dvoid**)0,(ub4*)0);
...
OTL_TMPL_OUT_STREAM& operator<<( ::std::vector<int>& s)
{

	OCINumber oci_number;
	
	OCIObjectNew((this->cursor_struct.db)->envhp,(this->cursor_struct.db)->errhp,(this->cursor_struct.db)->svchp,
		OCI_TYPECODE_VARRAY,this->cursor_struct.eleminfo_tdo,(dvoid*)NULL,OCI_DURATION_SESSION,FALSE,
		(dvoid**)&(this->vl[cur_x+1]->var_struct.hyarray[cur_y]));
	int len = (int)s.size();

	for (int i=0;i<len;i++)
	{
		OCINumberFromInt((this->cursor_struct.db)->errhp,(dvoid*)&s[i],(uword)sizeof(int),OCI_NUMBER_UNSIGNED,&oci_number);
		OCICollAppend((this->cursor_struct.db)->envhp,(this->cursor_struct.db)->errhp,
			(dvoid*)&oci_number,(dvoid*)0,(OCIColl*)(this->vl[cur_x+1]->var_struct.hyarray[cur_y]));
	}
	if(cur_y ==0)
	OCIBindObject(this->vl[cur_x+1]->var_struct.bind,(this->cursor_struct.db)->errhp,
		this->cursor_struct.eleminfo_tdo,(dvoid**)&(this->vl[cur_x+1]->var_struct.hyarray[0]),(ub4*)0,(dvoid**)0,(ub4*)0);
...
public:
 ...
 OCIDescribe *dschp;// add by yjq
 //fix
 //OCINumber         global_gid[ARRAY_SIZE];
 //OCIType           *geom_tdo = NULL;
 ...
 char* xa_server_internal_name;
  ...
  static int initialize(const int threaded_mode=0)
  {
    ...
    int mode;
    if(threaded_mode)
      mode=OCI_THREADED | OCI_OBJECT;//mode=OCI_THREADED
    else
      mode= OCI_DEFAULT;//OCI_OBJECT;// mode=OCI_DEFAULT
 ...
 {int& status=last_status;
  ...
  status=OCIEnvInit
   (OTL_RCAST(OCIEnv**,&envhp),
    OCI_THREADED|OCI_OBJECT,//OCI_DEFAULT,
    0,
    0);
	*/
  status=OCIEnvCreate
...
public:
  ...
  // fix for select 
  ::std::vector<SDO_GEOMETRY_TYPE*> v_geom_obj;
  ::std::vector<SDO_GEOMETRY_ind*> v_geom_ind;

  // fix for insert
  ::std::vector<OCIArray*> hyarray;
  OCIBind* bind;
 
  OCIEnv *envhp; // OCI environment handle
  OCIError *errhp; // OCI Error handle

  SDO_GEOMETRY_BIN geometry_bin;
  // fix
...
// fix  for select
	void setEnv(OCIEnv *envhp1,OCIError *errhp1)
	{
         envhp = envhp1;
		 errhp = errhp1;
	}
// fix
 ...
 {int i;
  ...
  // fix for select
  int len = (int)v_geom_obj.size(); 
  if(len > 0 && select_stm_flag == true)
  { 
	  for(int i=0; i<len; i++)
	  {
		  if(v_geom_obj[i])
		  {
			  OCIObjectFree(envhp, errhp, (dvoid *)v_geom_obj[i], 
				  (ub2)OCI_OBJECTFREE_FORCE);
		  }
	  /*
      OCIObjectFree(envhp, errhp, (dvoid *)v_geom_ind[0], 
		   (ub2)OCI_OBJECTFREE_FORCE);*/
	  }
  ...
  const int apl_tab_flag=0)
 {
  ...
  }else if(ftype==otl_var_blob||ftype==otl_var_clob)
  {
   ...
   p_v=new ub1[elem_size*OTL_SCAST(unsigned,array_size)];
   memset(p_v,0,elem_size*OTL_SCAST(unsigned,array_size));
   // fix for select/insert
   if(ftype == otl_var_geometry)
   {
     if(select_stm_flag == true)
	 {
		 
         v_geom_obj.resize(array_size);
         v_geom_ind.resize(array_size);
 ...
 void* val(int ndx,int pelem_size)
 {
   ...
   // fix for select
   case otl_var_geometry:
	   geometry_bin.p_obj = v_geom_obj[ndx];
       geometry_bin.p_ind = v_geom_ind[ndx];
       return  OTL_RCAST(void*,&geometry_bin);
   // fix
   default:
     return OTL_RCAST(void*,&p_v[OTL_SCAST(unsigned,ndx)*pelem_size]);
   }
 ...
 static int int2ext(int int_type)
 {
  ...
  /* fix for Geometry */
  case inGeometry: return extGeometry;
  default:
   return otl_unsupported_type;
  }
 }
...
public:
  ...
  // fix for insert/update
 // OCIBind* bind1;
 // ::std::vector<OCIBind*> vectorBind;//vector<double>绑定
 // int  bindCount;
  OCIType *eleminfo_tdo;
...
//转换otl类型到oci类型
 ...
 // fix
 OCIType* get_tdo(const char *szname, OCIDescribe *dschp = NULL)
 {  
	 ...
	 status = OCIDescribeAny((this->db)->svchp, (this->db)->errhp, (text*)szname,(ub4)strlen((char*)szname),OCI_OTYPE_NAME,(ub1)1,(ub1)OCI_PTYPE_TYPE,dschp);
	 status = OCIAttrGet((dvoid *)dschp, (ub4)OCI_HTYPE_DESCRIBE,(dvoid *)&paramp, (ub4 *)0,(ub4)OCI_ATTR_PARAM, (this->db)->errhp);  
	 status = OCIAttrGet((dvoid *)paramp, (ub4)OCI_DTYPE_PARAM,(dvoid *)&type_ref, (ub4 *)0,(ub4)OCI_ATTR_REF_TDO, (this->db)->errhp);  
	 status = OCIObjectPin((this->db)->envhp, (this->db)->errhp, type_ref, (OCIComplexObject *)0,OCI_PIN_ANY, OCI_DURATION_SESSION,OCI_LOCK_NONE, (dvoid **)&tdo); 

	 if (!tdo)  
	 {   
		 ::std::cout<<"NULL TDO RETURNED FOR TYPE %s.\n"<<::std::endl;
		 exit(-1);
	 }
...
/*********************oci bind*********************************/
 int bind(const char* name,otl_var& v,const int elem_size,const int ftype,
  const int /*param_type*/,
  const int /*name_pos*/,
  const int /*connection_type*/,
  const int apl_tab_flag)
  ...
  int db_ftype=0;
   /***************fix for geometry******************/
	if(ftype==otl_var_geometry)
	{
        if(bindFlag == 0)
		{
			 dschpStatus = OCIHandleAlloc(OTL_RCAST(dvoid*,(this->db)->envhp), (dvoid **)&dschp,(ub4)OCI_HTYPE_DESCRIBE, (size_t)0,(dvoid **)0);
			 ordinates_tdo = get_tdo("MDSYS.SDO_ORDINATE_ARRAY",dschp);
			 eleminfo_tdo = get_tdo("MDSYS.SDO_ELEM_INFO_ARRAY",dschp);
			 bindFlag = 1;
		}
...
#endif
// fix for select
   if(db_ftype == extGeometry)
   {
	   if(bindFlag == 0)
	   {
		   dschpStatus = OCIHandleAlloc((this->db)->envhp, (dvoid **)&dschp,(ub4)OCI_HTYPE_DESCRIBE, (size_t)0,(dvoid **)0);
		   geom_tdo = get_tdo("MDSYS.SDO_GEOMETRY",dschp);
		   v.setEnv(this->db->envhp,errhp);
		   bindFlag = 1;
	   }
	status=OCIDefineByPos(cda, &defnp, errhp, 
			OTL_SCAST(ub4,column_num), 
		   (dvoid *)0, (sb4)0, SQLT_NTY, (dvoid *)0,
		   (ub2 *)0, (ub2 *)0, (ub4)OCI_DEFAULT);

	status = OCIDefineObject(defnp, errhp, geom_tdo, 
		   (dvoid **)&(v.v_geom_obj[0]), (ub4 *)0, 
		   (dvoid **)&(v.v_geom_ind[0]), (ub4 *)0);
    if(status)   
		return 0;
	return 1;
   }
...
#if defined(OTL_GEOMETRY)
 otl_inout_stream& operator<<( ::std::vector<int>& v)
 {
	/* OCINumber oci_number;
	 int    status = OCIObjectNew((this->cursor_struct.db)->envhp,(this->cursor_struct.db)->errhp,(this->cursor_struct.db)->svchp,OCI_TYPECODE_VARRAY,this->cursor_struct.eleminfo_tdo,(dvoid*)NULL,OCI_DURATION_SESSION,FALSE,(dvoid**)&hyarray[indexBind]);
	 int len = v.size();

	 for (int i=0;i<len;i++)
	 {
		 status = OCINumberFromInt((this->cursor_struct.db)->errhp,(dvoid*)&v[i],(uword)sizeof(int),OCI_NUMBER_UNSIGNED,&oci_number);
		 status = OCICollAppend((this->cursor_struct.db)->envhp,(this->cursor_struct.db)->errhp,(dvoid*)&oci_number,(dvoid*)0,(OCIColl*)hyarray[indexBind]);
	 }
	 OCIBindObject(this->cursor_struct.vectorBind[indexBind],(this->cursor_struct.db)->errhp,this->cursor_struct.eleminfo_tdo,(dvoid**)&(hyarray[indexBind]),(ub4*)0,(dvoid**)0,(ub4*)0);
	 if(++indexBind == this->cursor_struct.bindCount)
	 {
		 indexBind = 0;
	 }*/

//#if defined(OTL_NO_TMPL_MEMBER_FUNC_SUPPORT)
//	 otl_ora8_inout_stream::operator<<(v);
//#else
	 otl_ora8_inout_stream::operator<<(v);
//#endif
	/* if(indexBind == 0)
	 {
		for(int i=0; i<this->cursor_struct.bindCount; i++)
			 status = OCIObjectFree((this->cursor_struct.db)->envhp,(this->cursor_struct.db)->errhp,(dvoid*)hyarray[i],(ub2)OCI_OBJECTFREE_FORCE);
	 }*/
	 return *this;
 }
#endif
...
#if defined(OTL_GEOMETRY)
 otl_inout_stream& operator<<( ::std::vector<double>& v)
{
  /*  OCINumber oci_number;
	OCIParam *paramp = NULL;  
	OCIRef *type_ref = NULL;  
	OCIType *tdo = NULL;
    int    status = OCIObjectNew((this->cursor_struct.db)->envhp,(this->cursor_struct.db)->errhp,(this->cursor_struct.db)->svchp,
		                         OCI_TYPECODE_VARRAY,this->cursor_struct.ordinates_tdo,
					             (dvoid*)NULL,OCI_DURATION_SESSION,FALSE,(dvoid**)&hyarray[indexBind]);
    int len = v.size();

	for (int i=0;i<len;i++)
	{
		status = OCINumberFromReal((this->cursor_struct.db)->errhp,(dvoid*)&v[i],(uword)sizeof(double),&oci_number);
		status = OCICollAppend((this->cursor_struct.db)->envhp,(this->cursor_struct.db)->errhp,
			                    (dvoid*)&oci_number,(dvoid*)0,(OCIColl*)hyarray[indexBind]);
	}

	OCIBindObject(this->cursor_struct.vectorBind[indexBind],(this->cursor_struct.db)->errhp,
		          this->cursor_struct.ordinates_tdo,(dvoid**)&hyarray[indexBind],(ub4*)0,(dvoid**)0,(ub4*)0);
...
// fix for select
 OTL_ORA_COMMON_READ_STREAM& operator>>(SDO_geometry& g)
	 OTL_THROWS_OTL_EXCEPTION
 {
	 last_oper_was_read_op=true;
	 switch(shell->stream_type){
...
//#endif
	 ...
	 OTL_TRACE_WRITE(n,"operator >>","sdo_geometry&")
		 inc_next_ov();
	 return *this;
 }
...
#endif
 /************************fix for geometry*************************/
 otl_stream& operator<<(::std::vector<int>& g)
	 OTL_THROWS_OTL_EXCEPTION
 {
	 last_oper_was_read_op=false;
	 reset_end_marker();
	 OTL_TRACE_READ(g,"operator <<","::std::vector<int>");
	 switch(shell->stream_type){
   case otl_no_stream_type:
	   break;
   case otl_inout_stream_type:
...
//	   (*ss)->operator<<(g);
//#else
//	   (*ss)->operator<<<::std::vector<int>,otl_var_geometry>(g);
//#endif
//	   break;
//   case otl_refcur_stream_type:
//#if defined(OTL_NO_TMPL_MEMBER_FUNC_SUPPORT)
//	   (*ref_ss)->operator<<(g);
//#else
//	   (*ref_ss)->operator<<<::std::vector<int>,otl_var_geometry>(g);
//#endif
//	   if(!(*ov)&&(*ref_ss)->sl) create_var_desc();
	   break;
	 }
 ...
 otl_stream& operator<<(::std::vector<double>& g)
	 OTL_THROWS_OTL_EXCEPTION
 {
	 last_oper_was_read_op=false;
	 reset_end_marker();
	 OTL_TRACE_READ(g,"operator <<","::std::vector<double>");
	 switch(shell->stream_type){
   case otl_no_stream_type:
	   break;
   case otl_inout_stream_type:
...
//	   (*ss)->operator<<(g);
//#else
//	   (*ss)->operator<<<::std::vector<double>,otl_var_geometry>(g);
//#endif
//	   break;
//   case otl_refcur_stream_type:
//#if defined(OTL_NO_TMPL_MEMBER_FUNC_SUPPORT)
//	   (*ref_ss)->operator<<(g);
//#else
//	   (*ref_ss)->operator<<<::std::vector<double>,otl_var_geometry>(g);
//#endif
//	   if(!(*ov)&&(*ref_ss)->sl) create_var_desc();
	   break;
	 }
